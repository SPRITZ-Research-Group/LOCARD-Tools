    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (layout != null) {
            ...
            if (lineCount > 0 && layout.getEllipsisCount(lineCount - 1) > 0) {
                setSingleLine(false);
                setMaxLines(2);
                TypedArray a = getContext().obtainStyledAttributes(null, j.TextAppearance, 16842817, 16973892);
                int textSize = a.getDimensionPixelSize(j.TextAppearance_android_textSize, 0);
                if (textSize != 0) {
