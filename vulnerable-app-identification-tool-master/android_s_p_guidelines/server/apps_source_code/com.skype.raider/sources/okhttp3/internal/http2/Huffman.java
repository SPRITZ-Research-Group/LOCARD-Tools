...
class Huffman {
    ...
    private static final class Node {
        ...
        Node() {
            this.children = new Node[Defines.SKYLIB_CONVERSATION_MAX_TOPIC_SIZE];
            this.symbol = 0;
            this.terminalBits = 0;
        }
    ...
    void encode(f data, d sink) throws IOException {
        long current = 0;
        int n = 0;
        for (int i = 0; i < data.h(); i++) {
            ...
            while (n >= 8) {
                n -= 8;
                sink.h((int) (current >> n));
            }
        }
        if (n > 0) {
            sink.h((int) ((current << (8 - n)) | ((long) (255 >>> n))));
        }
    }

    int encodedLength(f bytes) {
        long len = 0;
        for (int i = 0; i < bytes.h(); i++) {
            len += (long) CODE_LENGTHS[bytes.a(i) & 255];
        }
        return (int) ((7 + len) >> 3);
    }
