...
public final class k {
    ...
    private boolean a(RandomAccessFile f) throws IOException {
        ...
        do {
            ...
            if (prev < f.length()) {
                ...
                g.a().b("StreamableVideoUtil", "read in " + atom_type + " at " + prev);
                if (atom_size == 1) {
                ...
                if (atom_type.equals(j)) {
                    this.o = atom_size;
                    g.a().b("StreamableVideoUtil", "ftyp atom size is " + this.o);
                    this.n = new byte[((int) this.o)];
                    if (f.read(this.n) == -1) {
                    }
                } else {
                    g.a().b("StreamableVideoUtil", "skipping " + atom_size);
                ...
                if (!atom_type.equals(a) && !atom_type.equals(b) && !atom_type.equals(c) && !atom_type.equals(d) && !atom_type.equals(e) && !atom_type.equals(f) && !atom_type.equals(g) && !atom_type.equals(h) && !atom_type.equals(i) && !atom_type.equals(j)) {
                    g.a().c("StreamableVideoUtil", "encountered non-QT top-level atom " + (atom_type == j ? " match " : " non"));
                }
            }
            if (atom_type.equals(d)) {
                g.a().b("StreamableVideoUtil", "seeking (load moov) " + (prev - atom_size));
            ...
            g.a().b("StreamableVideoUtil", "last atom was not a moov atom but " + atom_type);
            f.close();
            return false;
        } while (atom_size >= 8);
        f.close();
    ...
    private void b(RandomAccessFile f) throws IOException {
        g.a().b("StreamableVideoUtil", "about to patch");
        int i = 4;
        while (((long) i) < this.q - 4) {
            ...
            if (atom_type.equals(k)) {
                g.a().b("StreamableVideoUtil", "patching STCO atom");
                ...
                g.a().b("StreamableVideoUtil", "patching co64 atom");
                atom_size = (long) ((int) a(Arrays.copyOfRange(this.p, i - 4, i)));
                if ((((long) i) + atom_size) - 4 > this.q) {
                    g.a().b("StreamableVideoUtil", "bad atom size");
                    f.close();
    ...
    private boolean a(RandomAccessFile f, RandomAccessFile out) throws IOException {
        ...
        f.read(temp_buf);
        g.a().b("StreamableVideoUtil", "read to " + f.getFilePointer() + " into " + this.q);
        out.seek(this.o);
        g.a().b("StreamableVideoUtil", "out pointer is at " + out.getFilePointer());
