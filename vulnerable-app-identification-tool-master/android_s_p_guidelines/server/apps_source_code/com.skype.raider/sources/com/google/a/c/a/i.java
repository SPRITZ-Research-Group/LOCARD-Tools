...
public final class i {
    ...
    static {
        ...
        for (int i = 0; i < 255; i++) {
            ...
            c[p] = i;
            p <<= 1;
            if (p >= Defines.SKYLIB_CONVERSATION_MAX_TOPIC_SIZE) {
                p ^= 301;
            }
        }
    }

    public static String a(String codewords, k symbolInfo) {
        if (codewords.length() != symbolInfo.f()) {
            throw new IllegalArgumentException("The number of codewords does not match the selected symbol");
        }
        StringBuilder sb = new StringBuilder(symbolInfo.f() + symbolInfo.g());
        sb.append(codewords);
        int blockCount = symbolInfo.a();
        if (blockCount == 1) {
            sb.append(a((CharSequence) codewords, symbolInfo.g()));
        } else {
            ...
            for (int i = 0; i < blockCount; i++) {
                dataSizes[i] = symbolInfo.a(i + 1);
                errorSizes[i] = symbolInfo.h();
                startPos[i] = 0;
                if (i > 0) {
            ...
            for (int block = 0; block < blockCount; block++) {
                ...
                for (int d = block; d < symbolInfo.f(); d += blockCount) {
                    temp.append(codewords.charAt(d));
                }
                String ecc = a(temp.toString(), errorSizes[block]);
                int pos = 0;
                int e = block;
                while (e < errorSizes[block] * blockCount) {
                    int pos2 = pos + 1;
                    sb.setCharAt(symbolInfo.f() + e, ecc.charAt(pos));
                    e += blockCount;
                    pos = pos2;
                }
            }
    ...
    private static String a(CharSequence codewords, int numECWords) {
        int i = 0;
        ...
        int[] iArr = b[i2];
        char[] cArr = new char[numECWords];
        for (i2 = 0; i2 < numECWords; i2++) {
            cArr[i2] = 0;
        }
        for (int i3 = 0; i3 < length + 0; i3++) {
            int charAt = codewords.charAt(i3) ^ cArr[numECWords - 1];
            i2 = numECWords - 1;
            while (i2 > 0) {
                if (charAt == 0 || iArr[i2] == 0) {
                    cArr[i2] = cArr[i2 - 1];
                } else {
                    cArr[i2] = (char) (cArr[i2 - 1] ^ d[(c[charAt] + c[iArr[i2]]) % 255]);
                }
                i2--;
            }
        ...
        char[] cArr2 = new char[numECWords];
        while (i < numECWords) {
            cArr2[i] = cArr[(numECWords - i) - 1];
            i++;
        }
